    ...
    <nav class="contenu">
        ...
        <script>
            ...
            function loadAffrontement(affrontement) {
                ...
                // Envoyer une requête AJAX pour vérifier si l'affrontement est terminé
                $.ajax({
                    type: "POST",
                    url: "check_affrontement_status.php", // Créez ce fichier pour vérifier le statut de l'affrontement
                    data: {tournament: selectedTournament, affrontement: affrontement},
                    success: function (status) {
                        console.log("Statut de l'affrontement " + affrontement + " : " + status);

                        // Si l'affrontement est terminé, n'affichez pas les boutons de vote
                        if (status === "termine") {
                            $("#participantsSection").html("<h2>Cet affrontement est terminé.</h2>");
                        } else {
                            // Sinon, continuez à charger les boutons de vote
                            $.ajax({
                                type: "POST",
                                url: "get_affrontement.php",
                                data: {tournament: selectedTournament, affrontement: affrontement},
                                success: function (response) {
            ...
            function confirmVotes() {
                ...
                // Récupérer les valeurs des votes
                var votesAnime1 = $("#voteFormAnime1 .votesInput").val();
                var votesAnime2 = $("#voteFormAnime2 .votesInput").val();

                // Vérifier si les deux cases sont remplies
                if (votesAnime1 !== "" && votesAnime2 !== "") {
                    // Confirmer les votes (Appeler la procédure stockée)
                    var selectedTournament = $("#selectedTournament").val();
                    var affrontement = 1; // Mettez le numéro d'affrontement correct ici

                    $.ajax({
                        type: "POST",
                        url: "confirm_votes.php", // Créez ce fichier pour gérer la confirmation des votes
                        data: {tournament: selectedTournament, affrontement: affrontement, votesAnime1: votesAnime1, votesAnime2: votesAnime2},
                        success: function (response) {
                            console.log("Réponse de la confirmation des votes : ", response);
                            // Ajoutez ici le traitement nécessaire après la confirmation des votes
                        }
                    });
                } else {
                    alert("Veuillez remplir les deux cases de votes.");
                }
            }
        </script>
